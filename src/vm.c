...
#define READ_STRING(reader) AS_STRING(reader())
...
#define BINARY_OP(valueType, op)                                               \
    ...
    for (;;) {
        ...
        case OP_GET_GLOBAL_LONG: {
            ObjString* name = READ_STRING(READ_CONSTANT_LONG);
            Value value;
            if (!tableGet(&vm.globals, name, &value)) {
                runtimeError("Undefined variable '%s'.", name->chars);
                return INTERPRET_RUNTIME_ERROR;
        ...
        case OP_INVOKE: {
            ObjString* method = READ_STRING(READ_CONSTANT);
            int argCount      = READ_BYTE();
        ...
        case OP_METHOD: defineMethod(READ_STRING(READ_CONSTANT)); break;
        }
    }
